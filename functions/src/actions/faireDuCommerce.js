...
//
// Selling an object of rarity "mythique" or above also seeds a rumor of the sale at the seller's
// current region (docs/TODO.md "Intermède actions": "selling a mythic object... spread the rumor
// of that object's presence at the sale location"): since a sale has no hand-authored
// worldData/rumors/items entry to copy text/rarity from (unlike RumorsManager.jsx's own flow),
// one is authored on the fly here, then seeded into
// worldData/regions/items/{regionId}/rumorSightings the same way RumorsManager.jsx seeds a
// hand-authored rumor - skipping the normal hop-by-hop propagation decay, per the spec.
//
...
const MYTHIC_SALE_RARITY_INDEX = RARITY_ORDER.indexOf("mythique");
...
async function resolve({ tx, db, character, characterId, context }) {
  ...
  let mythicRumorTriggered = false;
  const regionId = character.region?.id;
  if (regionId && RARITY_ORDER.indexOf(object.rarity) >= MYTHIC_SALE_RARITY_INDEX) {
    const rumorRef = db.collection("worldData").doc("rumors").collection("items").doc();
    const sightingRef = db
      .collection("worldData")
      .doc("regions")
      .collection("items")
      .doc(regionId)
      .collection("rumorSightings")
      .doc(rumorRef.id);

    tx.set(rumorRef, {
      text: `On raconte qu'un certain ${object.name} a changé de mains à ${character.region?.name || "cette région"}.`,
      rarity: object.rarity,
      originRegionIds: [regionId],
      linkedQuestId: null,
    });
    tx.set(sightingRef, { rarity: object.rarity, arrivedAt: FieldValue.serverTimestamp() });
    mythicRumorTriggered = true;
  }
