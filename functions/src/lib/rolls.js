...
// table (e.g. partirExplorer.js's per-round difficulty draw) is expected to hand it a table it
// knows is non-empty - there is no shared "empty pool" handling above this any more since the
// framework-wide tiers mechanism that used to need one was retired (see "Abandoning the paliers
// system" in docs/ISSUE-02-ACTION-FRAMEWORK.md).
...
// The mission/quest 6-tier difficulty scale (see src/lib/difficulties.js's DIFFICULTIES) -
// positionally equivalent to the first six rarity tiers (docs/TODO.md "Quest difficulty") but kept
// as its own array since it has no "divin"/"unique" tier.
