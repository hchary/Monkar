> **Archived on 2026-08-24.** This is the roadmap and backlog as it stood before the Python-model
> rework ([rework-plan.md](../../rework-plan.md)). The live roadmap is [docs/TODO.md](../TODO.md),
> which starts over from that plan. Everything below is kept as the design record for what was
> already built — the `Status: **implemented**` entries are still the reference documentation for
> those systems, and the roadmap table's `done` rows are the history of how they landed.
>
> Four rows were still open when this file was archived and were **not** carried into the new
> roadmap: #32 (Métier action-kind polish), #33 (misc small polish), #34 (rumor region-to-region
> propagation — moot, the rumor system was retired in `d05821c`), #35 (known recipes grant
> mechanism — implementation). Re-add them to the live roadmap if they are still wanted.
