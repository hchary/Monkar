...
- **`TagsManager.jsx`**: CRUD for the standalone `worldData/tags/items` label catalog (name only), alphabetically sorted, editable via a popup dialog (edit or delete). Deleting a tag first strips its id from every quest's and every narrative subject's `tagIds` (via a `tagIds array-contains` query against each collection) before deleting the tag doc, since `QuestsManager.jsx` and `QuestObjectivesManager.jsx` are the two consumers that reference tag ids.
