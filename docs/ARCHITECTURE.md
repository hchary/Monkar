...
- **`TagsManager.jsx`**: CRUD for the standalone `worldData/tags/items` label catalog (name only), alphabetically sorted, editable via a popup dialog (edit or delete). Deleting a tag first strips its id from every quest's, narrative subject's, and object's `tagIds` (via a `tagIds array-contains` query against each collection) before deleting the tag doc, since `QuestsManager.jsx`, `QuestObjectivesManager.jsx`, and `ObjectsManager.jsx` are the consumers that reference tag ids.
