...
characters/{characterId}
  ...
  reputation: number                -- LEGACY, superseded by `reputations` below: a single global
                                     -- score, starting at background.reputationStart. Kept in step
                                     -- with the map below, and still the one every UI reads today
  reputations: { [regionId]: number } -- per-region reputation (docs/TODO.md "Per-region
                                     -- reputation"). Written by every action's reputation gain
                                     -- (functions/src/lib/actionResult.js); not read by any UI yet
...
- `resolve({ tx, db, character, actionType, today, context })` — runs **inside** the transaction, after the once-per-day lock re-check. Returns `{ updates, logFields }`: `updates` is the full `characters/{id}` patch (`lastActionDate`, `lastActionAt`, `lastAction`, plus whatever else the handler decides to write), `logFields` is the handler-specific subset merged into the `actionsLog` entry (the dispatcher adds the common `characterId`/`ownerUid`/`actionTypeId`/`date`/`createdAt` fields itself). The half of that patch that several handlers share — loot, talents, reputation, wounds, the character's region — is not hand-rolled per handler: it goes through `ActionResult` and its single applier (below). Everything else (gold, professions, the mission journal, chain progress, the Intermède counter) is handler-specific state the handler still writes itself.
...
### `functions/src/lib/actionResult.js`: the effect vocabulary and its single applier

Every effect more than one handler produces is named once here and written once (docs/TODO.md "ActionResult and the single applier"). `createActionResult({ itemsGained, itemsLost, talentsGained, talentTrained, reputationGained, reputationRegionId, newRegionId, injury })` builds the outcome a handler's `resolve()` produces; `applyActionResult(character, result, { today, circumstance, talentCatalog, regionName })` turns it into `{ updates, died }`. Pure — no Firestore, no catalog reads: anything the applier cannot know from the character document arrives through the options bag.

- **`itemsGained` / `itemsLost`** are loot entries (or bare object ids, normalized to `{ objectId }`). Neither touches `instances`: gains are frozen onto `lastAction.loot` and materialized by the handler's own `commit()` once the player acknowledges the result (the anti-duplication guarantee), and losses are recorded for display while the deletion stays in the handler's transaction, where the reads-before-writes ordering it needs is available.
- **`talentTrained`** bumps an owned talent one quality step (`bumpTalentQuality`); **`talentsGained`** grants a catalog talent at quality 1. An id already owned, or missing from `talentCatalog`, is skipped rather than throwing; a repeated id is applied once. Both land on `lastAction.talentEvolutions` as `{ talentId, name, kind: "evolution" | "unlock", quality, rarity }`.
- **`reputationGained`** is signed and credited to `reputationRegionId`, or to the region the character stands in *after* `newRegionId` is applied, into `character.reputations`; a missing entry defaults to 0. INTERIM: the legacy `character.reputation` scalar is kept in step until docs/TODO.md "Per-region reputation" moves `minReputation` and the banner onto the map. A gain with no region to land in is dropped rather than written under an empty key.
- **`newRegionId`** writes `character.region` (name from the `regionName` option) and seeds an unvisited region's reputation at 1. Nothing sets it until docs/TODO.md "Travel action (Voyager)".
- **`injury`** is the engine's `{ light, severe, permanent }` triple, collapsed to a severity and routed through `wounds.js`'s `applyWound` — closing the harm chain at the one place that knows the escalation ladder. `died` is returned and `alive: false` written.

The patch only carries what the result actually moved: an empty result returns `{}`. `updates.lastAction` holds the player-visible summary of those same effects, which the handler spreads into its own `lastAction` alongside the fields only it knows (`success`, `score`, `mission`, ...). Two fields of the source model are deliberately not ported (`idleTime` — the web has `stampLifecycle`; `talentsLost` — nothing can take a talent away) and one is added (`reputationRegionId`, so an action spanning regions can name the one it credits). `apprentissage.js`, `recherche.js` and `faireDuCommerce.js` build no `ActionResult` at all: their whole effect set (a profession grant, the mission journal, gold plus the Intermède counter) falls outside the vocabulary, so routing them through it would apply nothing.

### `functions/src/lib/monsters.js`: resolving the bestiary's inheritance

A monster's `parentId` chain resolved the way `shared/schema/monster.ts` documents it: array fields (`tagIds`, `lootItemIds`) concatenate down the chain deduplicated, ancestors first; scalars (`difficulty`, `areaType`, `talentRewardId`) take the first value the chain carries, child first; `name` and `trigger` are never inherited. `monsterChain` caps at `MAX_INHERITANCE_DEPTH` (8) and survives an authored cycle. `monstersForAreaType(monsters, areaType)` is the draw pool: every monster whose *resolved* `areaType` matches, so a variant inherits its parent's area; a null area type matches nothing. Mirrored (resolution half only) in `src/lib/monsters.js`, which feeds the creator's "hérité du parent" panel; this copy is the tested one.

### `functions/src/lib/missionResolution.js`: the resolution engine

Pure math, no Firestore, shared by every Aventure-branch encounter. One `rollD100` (domain **0..99**), raised by the character's relevant talents, compared against a single threshold indexed by difficulty position (`SUCCESS_THRESHOLD = [10, 40, 70, 90, 95, 100]`, `DIFFICULTY_ORDER` order); the same raised roll is then read against that tier's injury bands (`INJURY_THRESHOLDS`, `light`/`severe`/`permanent`, exclusive, most severe first, `<=`). `resolveMission({ character, tagIds, difficulty })` returns `{ roll, relevantSum, updatedRoll, difficultyIndex, effectiveDifficultyIndex, threshold, success, injury, wound }`.

- **Talents raise the roll, they don't lower the bar.** `checkAgainstTalents` sums the `quality` of every talent sharing a tag with the mission *and* reaching the mission's difficulty index — a weak talent contributes nothing at a high tier.
- **Quality-5 talents also drop tiers.** `updateDifficulty` spends them one whole tier at a time, each step costing the mission's *original* difficulty index and needing strictly more than that left over; the result floors at `facile`, and a single quality-5 talent never drops a tier at any difficulty. The dropped tier drives both the success threshold and the injury bands.
- **Two rules are encoded, not emergent**: `mythique`'s threshold of 100 sits outside the roll's domain, so it is unwinnable without talents (`isWinnableWithoutTalents`, `MAX_ROLL`); and every tier's light band sits strictly below its own success threshold, so a success mathematically never wounds (the injury is still returned unconditionally, and both properties are asserted by `missionResolution.test.js`).
- **Wound application is elsewhere**: the injury triple collapses to the severity string `functions/src/lib/wounds.js`'s `applyWound` takes, and `characters/{id}` keeps its three flat counters.
...
The move from the subject/action mission catalogs to the Area + bestiary content model (docs/TODO.md "Content migration scripts") ships as four one-off scripts under `functions/scripts/`, plus one destructive cleanup held back until the generation pass is repointed. Like every script in that directory they authenticate through Application Default Credentials and are **generated, reviewed and run by hand** — no app code invokes them, and each is idempotent, so a re-run never clobbers authoring done in between.

| Script | Does | Run when |
|---|---|---|
| `seedAreasFromRegions.js` | One `worldData/areas/items` document per distinct region `climateIds`/`reliefIds` combination, then sets each region's `areaId`. The Area `type` is guessed from the climate's `bannerKey`, then from keywords in the climate/relief names, falling back to `plaine`; every guess is printed under "TO REVIEW" | Any time |
...
- **`RegionsManager.jsx`**: CRUD for `worldData/regions/items`, and per-region CRUD for the nested `backgrounds` subcollection (expand a region to manage its own background pool inline). Its `areaId` is a nullable single-select (`SoloSelectModalField.jsx`) over `worldData/areas/items`, tooltipped with each Area's type and shown in the region list — the link mission generation will filter the bestiary through.
- **`AreasManager.jsx`**: CRUD for `worldData/areas/items` — see the shape above. Registered as the "Zones" tab under the "Carte" group, next to "Régions". Name, a `type` select over `AREA_TYPES`, and `tagIds` / `lootTableIds` multi-selects; the same list + `details` form-panel shape as every other manager, with no styling of its own.
...
- **`MonstersManager.jsx`**: CRUD for `worldData/monsters/items` — see the shape above. Registered as the "Monstres" tab under the "Missions" group, now its only tab — the `MissionActionsManager.jsx` / `MissionSubjectsManager.jsx` tabs it replaces were deleted by docs/TODO.md "Content migration scripts", which carries their content over. Name, a `difficulty` select over `DIFFICULTIES`, an `areaType` select over `AREA_TYPES` with an "Hériter du parent" empty option, a `parentId` single-select that excludes the monster itself and every descendant (client-side cycle prevention), plus `tagIds` / `lootItemIds` multi-selects and a nullable `talentRewardId` single-select. Its read-only "Hérité du parent" panel resolves the parent chain live so an author sees what a monster actually carries without opening its ancestors: `src/lib/monsters.js` (`monsterChain`, `resolveMonster`, `resolveInheritedFrom`, `selfAndDescendantIds`) is the client half of the twin the generation pass will need server-side, same convention as `actionConditions.js` / `lootTables.js`. `trigger` has no control — it is authored by script and simply round-tripped on save.
- **`TagsManager.jsx`**: CRUD for the standalone `worldData/tags/items` label catalog (name only), alphabetically sorted, editable via a popup dialog (edit or delete). Deleting a tag first strips its id from every mission subject's, object's, loot table's, and recette's `tagIds` (and a recette's `categoryIds`, since it draws from the same catalog) — via a `tagIds`/`categoryIds` `array-contains` query against each collection — before deleting the tag doc, since `ObjectsManager.jsx`, `TablesDeTirageManager.jsx`, and `RecettesManager.jsx` are the consumers that reference tag ids. STALE: the sweep still queries the retired `quests` / `narrativeSubjects` / `verbPhrases` collections and does not yet query `monsters` or `areas`, so a deleted tag can leave a dangling id on a monster. `ObjectsManager.jsx` similarly strips a deleted object's id from every loot table's `itemIds` and every recette's `ingredients`/`results` entries before deleting the object (the latter can't use an `array-contains` query since each entry is a `{objectId, qty}` map, not a bare id, so it fetches all recettes and filters client-side instead).
- **`MultiSelectModalField.jsx`**: a shared, catalog-agnostic multi-select control — a `<dialog>` popup with a text-filtered checkbox list and selected-item chips, used wherever a form needs to pick several items out of a potentially large catalog. Its search behavior isn't hardcoded: a `matchesFilter(option, query)` prop decides what the popup's text filter actually matches against (defaults to a plain name match), and its trigger button label defaults to "Choisir" but can be overridden via `buttonLabel` (e.g. tags fields across several managers use "Ajouter tags"). Each catalog's own manager exports a matching function shaped for its data — `matchesRegion` (`RegionsManager.jsx`, matches name or description), `matchesTalent` (`TalentsManager.jsx`), `matchesTag` (`TagsManager.jsx`), `matchesObject` (`ObjectsManager.jsx`, matches name/description/rarity), `matchesOrigin` (`OriginsManager.jsx`, matches name/description), `matchesRecette` (`RecettesManager.jsx`, matches name/rarity), `matchesMonster` (`MonstersManager.jsx`, matches name/difficulty/area type), `matchesArea` (`AreasManager.jsx`, matches name or area type, key or label) — so any future `MultiSelectModalField` usage against that catalog reuses the same filter instead of redefining it.
