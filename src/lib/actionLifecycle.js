...
// functions/src/lib/actionLifecycle.js, so the client can decide what to render and the Cloud
// Functions can decide what to authorize from the exact same rules. functions/ is CommonJS with
// no build step shared with the Vite app, so a duplicated pure module is the established answer
// here - same convention as src/lib/lootTables.js vs functions/src/lib/loot.js. The functions
// copy is the one covered by tests (actionLifecycle.test.js); keep this one in step when editing.
//
...
export function actionCompletesAtMillis(character) {
...
export function isActionRunning(character, nowMillis = Date.now()) {
...
export function isActionAcknowledged(character) {
  const lastAction = character?.lastAction;
...
// lastAction to recap).
export function actionState(character, nowMillis = Date.now()) {
