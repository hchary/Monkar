...
// The player-facing side of the "Mission" action: unlike every other action in ActionBrowser, it
// needs a payload (missionId) before it can start - so ActionBrowser swaps its plain "Commencer"
// button for this list of the character's currently offered character.missionJournal entries
// (generated by the "Se renseigner" action, functions/src/actions/recherche.js), same convention
// as CraftingTab.jsx passing recetteId through to the "artisanat" handler.
export default function MissionPicker({ character, onStart, submitting, availabilityOk }) {
  ...
  // A mission resolved (or replaced by a fresh Se renseigner roll) out from under the current
  // selection can't stay selected - same pattern as CraftingTab's own selectedId effect.
  useEffect(() => {
  ...
  return (
    <div className="mission-picker">
      <fieldset className="action-loot-box">
        ...
        {missions.length === 0 ? (
          <p className="empty-state">Aucune mission disponible — effectuez une recherche pour en générer.</p>
        ) : (
