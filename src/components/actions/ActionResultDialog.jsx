...
// Which worldData/quests/items ids this character's result pop-up has already shown on its
// "newly triggered quests" page, kept client-side (see character.triggeredQuestIds's own
...
function loadShownTriggeredQuestIds(characterId) {
  try {
    const raw = localStorage.getItem(`shownTriggeredQuests:${characterId}`);
...
function markTriggeredQuestsShown(characterId, questIds) {
  if (!questIds.length) return;
  const shown = loadShownTriggeredQuestIds(characterId);
  for (const id of questIds) shown.add(id);
  try {
    localStorage.setItem(`shownTriggeredQuests:${characterId}`, JSON.stringify([...shown]));
  } catch {
    // localStorage unavailable (private mode, quota) - the quest just stays "new" next time.
  }
}
...
// action's result (unchanged), page 2 lists quests the scheduled trigger sweep has granted since
// this dialog last showed them (only present when there are any), page 3 is a reserved, always-
// present placeholder for a messaging feature that doesn't exist yet. Paging is optional browsing
...
export default function ActionResultDialog({ lastAction, showLoot, character, onClose, closing, error }) {
  ...
  const [pendingQuestIds] = useState(() => {
    const shown = loadShownTriggeredQuestIds(character.id);
    return (character.triggeredQuestIds || []).filter((id) => !shown.has(id));
  });
  const [triggeredQuests, setTriggeredQuests] = useState([]);
  ...
  useEffect(() => {
    if (pendingQuestIds.length === 0) return;
    let cancelled = false;
    Promise.all(pendingQuestIds.map((id) => getDoc(doc(db, "worldData", "quests", "items", id)))).then((snaps) => {
      if (cancelled) return;
      setTriggeredQuests(snaps.filter((snap) => snap.exists()).map((snap) => ({ id: snap.id, ...snap.data() })));
    });
    return () => {
      cancelled = true;
    };
  ...
  function handleClose() {
    markTriggeredQuestsShown(character.id, pendingQuestIds);
    onClose();
  }

  const ResultComponent = RESULT_COMPONENTS[lastAction.handlerId] ?? DefaultResult;
  const hasQuestsPage = triggeredQuests.length > 0;
  ...
  return (
    <dialog ref={dialogRef} className="action-result-dialog" onCancel={(e) => e.preventDefault()}>
      <div className="action-result-content">
        ...
        {page === 2 && hasQuestsPage && (
          <>
            <h3>Nouvelles quêtes</h3>
            <ul>
              {triggeredQuests.map((quest) => (
